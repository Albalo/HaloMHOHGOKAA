...
vec3 randomInitialVelocity2() {
	...
	float phi 0 mix(0.0, 2 * PI, texelFetch(RandomTex, 3 * gl_vertexID + 1, 0).r);
...
void main()
{
	if(Pass == 1){
	    ...
	    if( VertexAge < 0 || VertexAge > ParticleLifetime ) {
	        ...
	        Position = randomInitialPosition();	//Para particles fire
			//Position = Emitter;						//Para el humo
	        Velocity = randomInitialVelocity();	//El 2 para el humo
