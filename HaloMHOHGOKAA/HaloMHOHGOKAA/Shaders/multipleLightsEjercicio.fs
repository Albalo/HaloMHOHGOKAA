...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	// Ambient
    vec3 ambient  = light.ambient * vec3(mix(texture(texture1, our_uv), texture(texture2, our_uv), 0.7));
  	
    // Diffuse 
    vec3 normal = normalize(our_normal);
    vec3 lightDir = normalize(-direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse  = light.diffuse * (diff * vec3(mix(texture(texture1, our_uv), texture(texture2, our_uv), 0.7)));
    ...
    vec3 specular = light.specular * (spec * vec3(mix(texture(texture1, our_uv), texture(texture2, our_uv), 0.7)));  
...
void main()
{
	vec4 colorText = mix(texture(texture1, our_uv), texture(texture2, our_uv), 0.7);
	if(colorText.a < 0.1)
		discard;
